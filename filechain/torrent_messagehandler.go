...
import (
	...
	"github.com/fichain/go-file/internal/cachedpiece"
...
func (t *torrent) handlePieceMessage(pm peer.PieceMessage) {
	...
	t.log.Debugf("piece #%d downloaded from %s", msg.Index, pe.Stream.Conn().RemotePeer().Pretty())
...
func (t *torrent) handlePeerMessage(pm peer.Message) {
	...
	case peerprotocol.HaveAllMessage:
		if t.pieces == nil || t.bitfield == nil {
			fmt.Println("have all message!, but queue!")
			pe.Messages = append(pe.Messages, msg)
			break
		}
		fmt.Println("have all message!, not queue!")
		if t.piecePicker != nil {
	...
	case peerprotocol.RequestMessage:
		...
		if !pi.Done {
			pe.Logger().Debugln("reject message!", msg.Index)
		...
		if pe.ClientChoking {
			if pe.FastEnabled {
				if pe.SentAllowedFast.Has(pi) {
					...
					pe.Logger().Debugln("reject message! not allow fast!", msg.Index)
					m := peerprotocol.RejectMessage{RequestMessage: msg}
					pe.SendMessage(m)
				}
			}
