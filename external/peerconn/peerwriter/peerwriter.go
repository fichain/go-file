...
func New(stream network.Stream, l logger.Logger, maxQueuedRequests int, fastEnabled bool, b *ratelimit.Bucket) *PeerWriter {
	return &PeerWriter{
		stream:              stream,
...
func (p *PeerWriter) messageWriter() {
	...
	err := p.stream.SetWriteDeadline(time.Time{})
	...
	for {
		...
		case msg := <-p.writeC:
			...
			n, err := p.stream.Write(buf.Bytes())
			if _, ok := msg.(Piece); ok {
		...
		case <-keepAliveTicker.C:
			_, err := p.stream.Write([]byte{0, 0, 0, 0})
			if _, ok := err.(*net.OpError); ok {
