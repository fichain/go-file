...
func New(s network.Stream, l logger.Logger, pieceTimeout time.Duration, b *ratelimit.Bucket) *PeerReader {
	return &PeerReader{
		stream:         s,
		r:            bufio.NewReaderSize(s, readBufferSize),
		log:          l,
...
func (p *PeerReader) Run() {
	...
	for {
		err = p.stream.SetReadDeadline(time.Now().Add(readTimeout))
		if err != nil {
			p.log.Debugln("errs:", err)
			return
		}
...
func (p *PeerReader) readPiece(length uint32) (buf bufferpool.Buffer, err error) {
	...
	for {
		...
		err = p.stream.SetReadDeadline(time.Now().Add(p.pieceTimeout))
		if err != nil {
			return
		}
